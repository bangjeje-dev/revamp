 ...
 * Modular Case Study Storytelling blocks, On-Demand Metadata Inspector, and Zero-Refresh Multi-Viewport Previews.
 */

class EditorialOS {
    static init(options = { isCaseStudy: false }) {
        this.isCaseStudy = options.isCaseStudy;
    ...
    static setupAutoSave() {
        ...
        document.addEventListener('input', (e) => {
            if (e.target && (e.target.classList.contains('editor-canvas') || e.target.closest('.storytelling-block') || e.target.classList.contains('canvas-title'))) {
    ...
    static updateWordCount() {
        const canvas = document.getElementById('editorial-canvas-root') || document.body;
    ...
    static executeSlash(type) {
        ...
        if (type === 'h2') {
            ...
            block.innerHTML = `
                ${gripHtml}
                <div class="p-4 rounded-xl bg-slate-900 text-white border border-slate-700 shadow-sm flex items-start gap-3.5 my-2">
                    <i class="ph ph-lightning text-xl text-[#C3FF00] shrink-0 mt-0.5"></i>
            ...
            block.innerHTML = `
                ${gripHtml}
                <blockquote class="pl-4 sm:pl-6 border-l-4 border-slate-900 my-4 py-1 italic font-serif text-lg text-slate-700 focus:outline-none" contenteditable="true">
        ...
        StudioToast.show(`Inserted slash block: "${type}"`, 'info', 'Block Engine');
    }
    ...
    static blockActionMenu(button) {
        ...
        // Simple visual demonstration of block commands
        const actions = ['Duplicate Block', 'Collapse Section', 'Move Up', 'Move Down', 'Delete Block'];
        const action = actions[Math.floor(Math.random() * actions.length)];
        
        if (confirm(`Execute block action on this section?\n\n• OK to Duplicate/Clone\n• Cancel to Delete Section`)) {
            const clone = block.cloneNode(true);
            block.after(clone);
            StudioToast.show('Section successfully duplicated.', 'success', 'Storytelling Block');
        } else {
            if (confirm('Delete this storytelling section?')) {
                block.remove();
    ...
    static setupBlockEvents() {
        // Allow clicking anywhere to append a paragraph block if canvas is empty
    ...
    static injectMetadataDrawer() {
        ...
        drawer.className = 'fixed inset-y-0 right-0 z-50 w-full sm:w-96 bg-white border-l border-slate-200 shadow-2xl transform translate-x-full transition-transform duration-200 flex flex-col';
        drawer.innerHTML = `
            <div class="h-16 px-6 border-b border-slate-200 flex items-center justify-between bg-slate-50/70 shrink-0">
                ...
                <button onclick="EditorialOS.closeMetadataDrawer()" class="text-slate-400 hover:text-slate-900 p-1 font-mono">✕</button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6 text-xs no-scrollbar">
                ...
                <div class="space-y-3 pt-4 border-t border-slate-200">
                    ...
                    <div class="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-2 font-medium">
                        ...
                        <button onclick="StudioToast.show('Linked new relational asset to graph.', 'info')" class="w-full py-1.5 rounded-lg bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 font-bold text-xs mt-1">+ Attach Related Content...</button>
                    </div>
                </div>
    ...
    static injectPublishModal() {
        ...
        modal.className = 'fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 hidden opacity-0 transition-all duration-150';
        modal.innerHTML = `
    ...
    static confirmBroadcast() {
        ...
        if (badge) {
            badge.className = 'px-2.5 py-1 rounded-md bg-emerald-100 text-emerald-800 font-mono text-[11px] font-extrabold uppercase tracking-wider';
    ...
    static injectViewportSandbox() {
        ...
        sandbox.className = 'fixed inset-0 z-50 bg-slate-900 text-white hidden flex flex-col transition-all';
        sandbox.innerHTML = `
            <header class="h-16 bg-slate-950 border-b border-slate-800 px-6 flex items-center justify-between shrink-0 text-xs font-mono">
                ...
                <div class="flex items-center gap-2 bg-slate-900 p-1.5 rounded-xl border border-slate-800">
                    <button onclick="EditorialOS.switchViewport('desktop')" id="vp-btn-desktop" class="px-3 py-1 rounded-lg bg-slate-800 text-[#C3FF00] font-bold">Desktop (1440p)</button>
                    <button onclick="EditorialOS.switchViewport('tablet')" id="vp-btn-tablet" class="px-3 py-1 rounded-lg hover:bg-slate-800 text-slate-400 font-semibold">Tablet (iPad Pro 11")</button>
                    <button onclick="EditorialOS.switchViewport('mobile')" id="vp-btn-mobile" class="px-3 py-1 rounded-lg hover:bg-slate-800 text-slate-400 font-semibold">Mobile (iPhone 15 Pro)</button>
                </div>
    ...
    static switchViewport(mode) {
        ...
        ['desktop', 'tablet', 'mobile'].forEach(m => {
            ...
            if (btn) {
                if (m === mode) btn.className = 'px-3 py-1 rounded-lg bg-slate-800 text-[#C3FF00] font-bold';
                else btn.className = 'px-3 py-1 rounded-lg hover:bg-slate-800 text-slate-400 font-semibold';
            }
        });
    ...
    // --- TEMPLATE SCAFFOLD LAUNCHER ---
    static loadTemplateScaffold(templateName) {
        const container = document.getElementById('editorial-canvas-blocks') || document.getElementById('editorial-canvas-root');
        if (!container) return;

        StudioToast.show(`Loaded template scaffold: "${templateName}". All modules remain 100% optional.`, 'success', 'Template Engine');
    }
}
